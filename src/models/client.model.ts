...
export const newClientSchema = Joi.object().keys({
  ...
  cpf: Joi.alternatives().conditional('tipoCliente', {
    not: 'PJ',
    then: Joi.string().required(),
    otherwise: Joi.any().allow(null),
  }),
  cnpj: Joi.alternatives().conditional('tipoCliente', {
    not: 'PF',
    then: Joi.string().required(),
    otherwise: Joi.any().allow(null),
  }),
  endereco: Joi.string().trim().required(),
  rendaAnual: Joi.alternatives().conditional('tipoCliente', {
    not: 'PJ',
    then: Joi.number().required(),
    otherwise: Joi.number(),
  }),
  faturamentoAnual: Joi.alternatives().conditional('tipoCliente', {
    not: 'PF',
    then: Joi.number().required(),
    otherwise: Joi.number(),
  }),
